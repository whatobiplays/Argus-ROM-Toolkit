-- Expand the durable content catalog while preserving every v11 row and
-- relationship. The rebuild is intentionally forward-only and runs inside
-- the migration runner's single IMMEDIATE transaction.
...
CREATE TABLE game_content (
    ...
    content_type TEXT NOT NULL CHECK (content_type IN ('CartridgeImage', 'MagneticDiskImage')),
    ...
    CHECK (
        ...
        OR
        (platform_id <> 'nintendo.fds' AND content_type = 'CartridgeImage')
    )
);
...
SELECT game_id, canonical_game_id, created_at
FROM game_redirect_legacy;
...
INSERT INTO external_identity_mapping (
    mapping_id, game_content_id, provider_id, external_game_id, external_release_id,
...
INSERT INTO resolved_metadata (
    game_id, display_title, sort_title, description, release_date, developers,
