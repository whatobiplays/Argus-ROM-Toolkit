...
#[test]
fn embedded_registry_applies_0002_sources_to_a_phase_000_database() {
    ...
    assert_eq!(second.migration_summary().current_version, 2);
    assert_eq!(
...
#[test]
fn fresh_database_reaches_migration_two() {
    let directory = tempdir().expect("tempdir");
    let executor = SqliteDatabaseExecutor::open(directory.path().join("argus.sqlite3"))
        .expect("fresh database");
    assert_eq!(executor.migration_summary().current_version, 2);
    assert_eq!(executor.migration_summary().applied_count, 2);
