...
void main() {
  test('late replacement subscription attaching after shutdown cannot hang '
    ...
    await client.runtime.resetAppearanceSettings(_idA);
...
final class _ControlledGateway implements ArgusClientGateway {
  ...
  Stream<RuntimeEvent> subscribeEvents(RuntimeInstanceId generation) {
    final task = _NativeTask(syncStreams, _admissionEpoch);
    tasks.add(task);
    return task.stream;
  }
...
final class _NativeTask {
  ...
  late final StreamController<RuntimeEvent> _controller;
  final bool _sync;
  final int _epoch;
  bool started = false;
  bool ended = false;

  _NativeTask(this._sync, this._epoch);
  ...
  Stream<RuntimeEvent> get stream {
    ...
    return Stream<RuntimeEvent>.multi((controller) {
      ...
      controller.onCancel = () => _taskEnded.future;
    });
  }

  void start({required bool boundaryClosed, required int admissionEpoch}) {
    started = true;
    if (boundaryClosed || _epoch != admissionEpoch) {
      _end();
    }
  }
