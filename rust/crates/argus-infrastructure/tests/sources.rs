...
fn embedded_registry_upgrades_a_phase_000_database_through_slice_004() {
    ...
    assert_eq!(second.migration_summary().current_version, 9);
    assert_eq!(
...
fn fresh_database_reaches_the_phase_001_schema() {
    ...
    assert_eq!(executor.migration_summary().current_version, 9);
    assert_eq!(executor.migration_summary().applied_count, 9);
